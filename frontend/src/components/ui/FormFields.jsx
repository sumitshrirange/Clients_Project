export const Field = ({ label, children, hint }) => (
  <label className="block">
    <span className="block text-sm font-semibold text-ink dark:text-canvas mb-1.5">{label}</span>
    {children}
    {hint && <span className="block text-xs text-ink-faint dark:text-canvas/50 mt-1">{hint}</span>}
  </label>
);
...
export const TextArea = (props) => (
  <textarea {...props} className={`${inputClass} min-h-[110px] resize-y ${props.className || ""}`} />
);
